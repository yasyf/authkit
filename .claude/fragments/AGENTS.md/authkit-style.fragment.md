...
Swift 6 language mode. Build with `swift build`, test with `swift test`, run with `swift run authkit`.

**Logic in the library, not the executable.** `Sources/authkit/` holds only the ArgumentParser command tree; everything it calls lives in `Sources/AuthKitLib/`, where tests can reach it. A command body longer than argument parsing + one library call is logic in the wrong target.
...
**Testing.** Tests live in `Tests/AuthKitLibTests/` and use Swift Testing — free `@Test` functions with `#expect`/`#require` against specific expected values, parameterized via `@Test(arguments:)`. Run them with `swift test`. Mock the boundaries the code talks to (network, filesystem, clock) and leave the function under test real.
